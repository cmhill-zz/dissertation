...
Since many downstream applications rely on these assembled genomese it is critical that the assembly is error-free.
...
As mentioned in the previous chapter, REAPR~\cite{hunt2013reapr} is a tool that leverages this constraint and evaluates the accuracy of the assembly using read-pair coverage.
...
Furthermore, if the \emph{correct} reference strain is not available, then QUAST maybe harmful and erroneously flag correct sequence as mis-assembled.
...
\section{Methods}
Regions of the genome are flagged based on inconsistencies in (1) sampling depth, (2) alignability of the sequences, and (3) insert size constraints.
...
VALET takes as input a metagenomic assembly \textsc{FASTA} file and a collection of paired and un-paired reads~\ref{fig:valet_pipeline}.
Assembled contigs are first filtered out based on size (by default 2x the average read length).
Next the abundance of contigs are calculated using our k-mer-based approach described above.
Contigs undergo an additional filtering step based on abundance (by default 10x).
Higher coverage and longer the sequence provide a better baseline for detecting mis-assemblies.
...
Therefore, any window of the assembly (by default 2000 in size) that contains multiple mis-assembly signatures are marked as \textbf{suspicious}.
